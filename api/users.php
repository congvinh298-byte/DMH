...
function verify_login_key_action(PDO $pdo, array $input): array
{
    ...
    // Decoding Logic: Extract ID based on prefix (Handles Complex Strings)
    ...
    // Direct Connection: 1. Check if it's a store
    if ($isStoreScan !== false && table_exists($pdo, 'marketplace_stores')) {
        ...
        if ($store) {
            // Immediate Error Feedback for Stores
    ...
    // Direct Connection: 2. Check if it's a user
    if ($isStoreScan !== true) {
