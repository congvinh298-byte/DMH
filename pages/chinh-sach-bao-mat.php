<?php
$page_title = 'Chính sách bảo mật';
$meta_title = 'Chính sách bảo mật thông tin - Chợ Lấp Vò Online';
require_once __DIR__ . '/inc/header.php';
?>

<h2>1. Mục đích và phạm vi thu thập thông tin</h2>
<p>Chợ Lấp Vò Online thu thập thông tin cá nhân của người dùng nhằm mục đích:</p>
<ul>
    <li>Xác thực danh tính người dùng (người mua, người bán).</li>
    <li>Hỗ trợ quá trình giao dịch, vận chuyển và thanh toán.</li>
    <li>Gửi thông báo về đơn hàng, khuyến mãi và cập nhật dịch vụ.</li>
    <li>Giải quyết tranh chấp, khiếu nại phát sinh.</li>
</ul>
<p>Các thông tin thu thập bao gồm: họ tên, số điện thoại, địa chỉ, email, thông tin đăng ký kinh doanh (nếu có).</p>

<h2>2. Phạm vi sử dụng thông tin</h2>
<p>Thông tin cá nhân của người dùng chỉ được sử dụng trong phạm vi:</p>
<ul>
    <li>Cung cấp cho người bán để thực hiện giao dịch (giao hàng, lắp đặt, bảo hành).</li>
    <li>Liên hệ xác nhận đơn hàng, hỗ trợ sau bán.</li>
    <li>Cung cấp cho cơ quan nhà nước có thẩm quyền khi có yêu cầu theo quy định pháp luật.</li>
</ul>

<h2>3. Thời gian lưu trữ thông tin</h2>
<p>Thông tin cá nhân của người dùng được lưu trữ cho đến khi người dùng yêu cầu xóa hoặc tự động xóa sau 02 năm kể từ lần hoạt động cuối cùng trên hệ thống, trừ trường hợp pháp luật có quy định khác.</p>

<h2>4. Cam kết bảo mật thông tin</h2>
<p>Chúng tôi cam kết:</p>
<ul>
    <li>Áp dụng các biện pháp kỹ thuật và tổ chức để bảo vệ thông tin cá nhân khỏi truy cập trái phép, mất mát, tiết lộ.</li>
    <li>Không bán, trao đổi hoặc cho thuê thông tin cá nhân của người dùng cho bên thứ ba vì mục đích thương mại.</li>
    <li>Chỉ cho phép nhân viên có thẩm quyền truy cập thông tin cá nhân trong phạm vi công việc được giao.</li>
</ul>

<h2>5. Quyền của người dùng</h2>
<p>Người dùng có quyền:</p>
<ul>
    <li>Tra cứu, chỉnh sửa thông tin cá nhân của mình trên hệ thống.</li>
    <li>Yêu cầu xóa thông tin cá nhân khỏi hệ thống.</li>
    <li>Báo cáo vi phạm bảo mật thông tin qua hotline 0979.553.289 hoặc email Congvinh28@gmail.com.</li>
</ul>

<h2>6. Đầu mối liên hệ bảo vệ quyền lợi người tiêu dùng</h2>
<p>Theo quy định tại Điều 39 Luật Bảo vệ quyền lợi người tiêu dùng:</p>
<ul>
    <li>Người đại diện được ủy quyền phối hợp với cơ quan nhà nước: <strong>Trần Công Vinh</strong> - Giám đốc</li>
    <li>Điện thoại: 0979.553.289</li>
    <li>Email: Congvinh28@gmail.com</li>
</ul>

<?php require_once __DIR__ . '/inc/footer.php'; ?>


<h3>Thông tin doanh nghiệp</h3>
<ul>
    <li><strong>Công ty:</strong> CÔNG TY TNHH MTV ĐIỆN MÁY HIẾU</li>
    <li><strong>Mã số thuế:</strong> 1402228630</li>
    <li><strong>Ngày thành lập:</strong> 06/04/2026</li>
    <li><strong>Email:</strong> Congvinh28@gmail.com</li>
    <li><strong>Hotline:</strong> 0979.553.289</li>
</ul>
